...
#--page 2 --depth 0 --output test.txt 'https://scholar.google.com/scholar?oi=bibs&hl=en&cites=7601154960238900186'
def getallresult():
    global driver
    driver = webdriver.Chrome()
    driver.close()
...
# Using readlines()
driver = webdriver.Chrome()
pubs = pd.read_csv('allPapers_citationNumbers.csv', header=0, nrows=24)
file1 = open('allpapers.txt', 'r')
...
for line in Lines:
    count += 1
    if line[0:2] == '% ':
        curentTitle = line[2:].strip()
        print(curentTitle)
    if line[0:6] == 'https:':
        currentinfo = line.split(" ")
        currenturl = currentinfo[0]
        currentcount = int(currentinfo[1])

        print(currenturl)
        tmp = currenturl.find("cites=")
        currentcluster = currenturl[(tmp+6):]
        with open(file2, 'a+') as f2:
                f2.write('%s###%s###%s\n' % (curentTitle, currentcount,currentcluster))
